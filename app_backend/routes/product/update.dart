...
Future<Response> onRequest(
  ...
  try {
    ...
    final userId =
        jwt.payload['id'].toString();

    final userRole =
        jwt.payload['role']?.toString() ??
            'customer';
    ...
    if (body['mainBannerImage'] != null) {
      updateData['mainBannerImage'] =
          body['mainBannerImage'].toString();
    }

    if (body['multipleImages'] != null &&
        body['multipleImages'] is List) {
      updateData['multipleImages'] =
          (body['multipleImages'] as List)
              .map((e) => e.toString())
              .toList();
    }
    ...
    if (body['discountPrice'] != null) {
      updateData['discountPrice'] =
          (body['discountPrice'] as num)
              .toDouble();
    }

    if (body['stock'] != null) {
      final stock =
          (body['stock'] as num).toInt();

      updateData['stock'] = stock;

      updateData['stockAvailable'] =
          stock > 0;
    }
