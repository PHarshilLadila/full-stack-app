...
// routes/product/review/list.dart - FIXED VERSION
...
Future<Response> onRequest(RequestContext context) async {
  ...
  try {
    ...
    // FIX: Convert ratingDistribution to Map<String, int> instead of Map<int, int>
    final ratingDistribution = await _getRatingDistribution(productId);

    // Convert to Map<String, int> for proper JSON serialization
    final serializableDistribution = <String, int>{};
    ratingDistribution.forEach((key, value) {
      serializableDistribution[key.toString()] = value;
    });
    ...
    final responseData = {
      ...
      'totalReviews': totalCount,
      'ratingDistribution':
          serializableDistribution, // Now it's Map<String, int>
      'reviews': transformedReviews,
      'pagination': {
        'currentPage': page,
        'totalPages': totalCount > 0 ? (totalCount / limit).ceil() : 1,
        'totalItems': totalCount,
        'itemsPerPage': limit,
      },
    };
    ...
    print('Rating distribution: $serializableDistribution');
