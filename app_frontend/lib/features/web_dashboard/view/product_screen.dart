...
class _ProductsContentState extends State<ProductsContent> {
            ...
            child: BlocProvider.value(
              ...
              child: BlocBuilder<ProductBloc, ProductState>(
                builder: (context, state) {
                  if (state is ProductLoading && _currentView == _CurrentView.productList) {
                    return const Center(child: CircularProgressIndicator());
                  } else if (state is ProductError && _currentView == _CurrentView.productList) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('Error loading products', style: TextStyle(fontSize: 18, color: Colors.grey.shade600)),
                          const SizedBox(height: 8),
                          Text(state.message, style: TextStyle(fontSize: 14, color: Colors.grey.shade500)),
                          const SizedBox(height: 16),
                          ElevatedButton(onPressed: () => productBloc.add(FetchSellerProductsEvent()), child: const Text('Retry')),
                        ],
                      ),
                    );
                  } else if (state is ProductLoaded || _currentView != _CurrentView.productList) {
                      ...
                      children: [
                        ElevatedButton(
                          ...
                          style: ElevatedButton.styleFrom(
                            ...
                            elevation: 0,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: const BorderSide(color: Colors.deepPurple, width: 0.8)),
                          ),
    ...
    return Padding(
      ...
      child: TextButton(
        ...
        style: TextButton.styleFrom(
          backgroundColor: _selectedFilter == filterValue ? const Color(0xFF7C3AED).withOpacity(0.1) : Colors.transparent,
          foregroundColor: _selectedFilter == filterValue ? const Color(0xFF7C3AED) : const Color(0xFF64748B),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        ),
        child: Text(label),
      ),
        ...
        children: [
          ...
          Container(
            ...
            child: DropdownButton<String>(
              ...
              items: categoryList.map((String category) => DropdownMenuItem<String>(value: category, child: Text(category))).toList(),
          ...
          Container(
            ...
            child: DropdownButton<String>(
              ...
              style: const TextStyle(color: Colors.black, fontSize: 14),
              items: _sortOptions.map((String option) => DropdownMenuItem<String>(value: option, child: Text(option))).toList(),
              onChanged: (String? newValue) => setState(() => _selectedSortBy = newValue!),
            ),
          ),
        ],
      ),
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            Text('No products found', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.grey.shade600)),
            const SizedBox(height: 8),
            Text('Try adjusting your search or add a new product', style: TextStyle(fontSize: 14, color: Colors.grey.shade500)),
          ],
        ),
      );
    }
                ...
                switch (column) {
                        ...
                        children: [
                          Container(
                            ...
                            child: ClipRRect(
                              ...
                              child: Image.network(product.mainBannerImage, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Container(color: const Color(0xFFF8FAFC), child: Icon(Icons.broken_image, color: Colors.grey, size: isSmallScreen ? 20 : 30))),
                            ),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(product.productName, style: TextStyle(fontWeight: FontWeight.w600, fontSize: isSmallScreen ? 12 : 14, color: const Color(0xFF1E293B)), overflow: TextOverflow.ellipsis, maxLines: 2),
                                const SizedBox(height: 4),
                                Text('ID: ${product.id}', style: TextStyle(fontSize: isSmallScreen ? 9 : 11, color: Colors.grey.shade500)),
                              ],
                            ),
                          ),
                        ],
                  ...
                  case 1:
                    return Padding(
                      ...
                      child: Text(product.category, style: TextStyle(fontSize: isSmallScreen ? 11 : 12, color: const Color(0xFF64748B), fontWeight: FontWeight.w500)),
                    );
                  case 2:
                    return Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
                      child: Text(product.isActive ? "Active" : "Inactive", style: TextStyle(fontSize: isSmallScreen ? 11 : 12, color: product.isActive ? Colors.green : Colors.grey, fontWeight: FontWeight.w500)),
                    );
                  case 3:
                    return Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                      child: Column(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('${product.stock} units', style: TextStyle(fontWeight: FontWeight.w500, fontSize: isSmallScreen ? 11 : 12, color: product.stock == 0 ? Colors.red : const Color(0xFF64748B))),
                          const SizedBox(height: 6),
                          Container(
                            padding: EdgeInsets.symmetric(horizontal: isSmallScreen ? 6 : 10, vertical: isSmallScreen ? 4 : 6),
                            decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                            child: Text(status, style: TextStyle(fontSize: isSmallScreen ? 10 : 12, color: statusColor, fontWeight: FontWeight.w600)),
                          ),
                        ],
                      ),
                    );
                  case 4:
                    return Padding(
                      ...
                      child: Column(
                        ...
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('₹${_formatPrice(product.discountPrice)}', style: TextStyle(fontWeight: FontWeight.w600, fontSize: isSmallScreen ? 12 : 14, color: const Color(0xFF1E293B))),
                          if (product.price > product.discountPrice) Text('₹${_formatPrice(product.price)}', style: TextStyle(fontSize: isSmallScreen ? 9 : 11, decoration: TextDecoration.lineThrough, color: Colors.grey.shade500)),
                        ],
                      ),
                    );
                  case 5:
                    return Row(
                      children: [
                        IconButton(onPressed: () => _showProductDetails(product), tooltip: 'View Details', iconSize: isSmallScreen ? 18 : 20, padding: EdgeInsets.zero, constraints: const BoxConstraints(), icon: const HugeIcon(icon: HugeIcons.strokeRoundedView, color: Color(0xFF64748B), size: 20.0)),
                        SizedBox(width: isSmallScreen ? 4 : 12),
                        IconButton(onPressed: () => _showEditProduct(product), tooltip: 'Edit Product', iconSize: isSmallScreen ? 18 : 20, padding: EdgeInsets.zero, constraints: const BoxConstraints(), icon: const HugeIcon(icon: HugeIcons.strokeRoundedEdit02, color: Color(0xFF7C3AED), size: 20.0)),
                        SizedBox(width: isSmallScreen ? 4 : 12),
                        IconButton(onPressed: () => _deleteProduct(product), tooltip: 'Delete Product', iconSize: isSmallScreen ? 18 : 20, padding: EdgeInsets.zero, constraints: const BoxConstraints(), icon: const HugeIcon(icon: HugeIcons.strokeRoundedDelete02, color: Color(0xFF64748B), size: 20.0)),
                      ],
                    );
                  default:
                    return const SizedBox();
            ...
            headerBuilder: (context, contentBuilder) {
              ...
              return contentBuilder(context, (context, column) {
                return Container(
                  ...
                  decoration: const BoxDecoration(color: Color(0xFFF8FAFC), border: Border(bottom: BorderSide(color: Colors.grey, width: 0.5))),
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                    child: Text(headers[column], style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF64748B))),
                  ),
                );
              });
            },
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Row(
                children: [
                  IconButton(onPressed: _showProductList, icon: const Icon(Icons.arrow_back)),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 24),
            _EditProductFormWidget(product: _selectedProduct!, onSuccess: _showProductList),
          ],
        ),
      ),
    );
      ...
      children: [
              ...
              ? Column(
                  ...
                  children: [
                    ...
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(product.productName, style: TextStyle(fontSize: isSmallScreen ? 20 : 24, fontWeight: FontWeight.bold, color: const Color(0xFF1E293B))),
                        const SizedBox(height: 8),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                          decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
                          child: Text(status, style: TextStyle(fontSize: 13, color: statusColor, fontWeight: FontWeight.w600)),
                        ),
                        const SizedBox(height: 16),
                        Wrap(
                          spacing: 16,
                          runSpacing: 8,
                          children: [
                            Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.category_outlined, size: 16, color: Color(0xFF7C3AED)), const SizedBox(width: 4), Text(product.category, style: const TextStyle(fontSize: 14, color: Color(0xFF64748B)))]),
                            Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.subdirectory_arrow_right, size: 16, color: Color(0xFF7C3AED)), const SizedBox(width: 4), Text(product.subCategory, style: const TextStyle(fontSize: 14, color: Color(0xFF64748B)))]),
                          ],
                        ),
                      ],
                    ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(product.productName, style: TextStyle(fontSize: isMediumScreen ? 22 : 24, fontWeight: FontWeight.bold, color: const Color(0xFF1E293B))),
                          const SizedBox(height: 8),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                            decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Pricing & Inventory', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
              const SizedBox(height: 16),
              isSmallScreen
                  ? Column(
                      children: [
                        _buildInfoCard('Price', '₹${_formatPrice(product.price)}', Icons.currency_rupee),
                        const SizedBox(height: 12),
                        _buildInfoCard('Discounted Price', '₹${_formatPrice(product.discountPrice)}', Icons.local_offer_outlined),
                        const SizedBox(height: 12),
                        _buildInfoCard('Stock', '${product.stock} units', Icons.inventory_2_outlined),
                      ],
                    )
                  : Row(
                      children: [
                        Expanded(child: _buildInfoCard('Price', '₹${_formatPrice(product.price)}', Icons.currency_rupee)),
                        const SizedBox(width: 16),
                        Expanded(child: _buildInfoCard('Discounted Price', '₹${_formatPrice(product.discountPrice)}', Icons.local_offer_outlined)),
                        const SizedBox(width: 16),
                        Expanded(child: _buildInfoCard('Stock', '${product.stock} units', Icons.inventory_2_outlined)),
                      ],
                    ),
            ],
          ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              const Text('Short Description', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF64748B))),
              const SizedBox(height: 8),
              Container(
                width: double.infinity,
                padding: EdgeInsets.all(isSmallScreen ? 12 : 16),
                decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12)),
                child: Text(product.shortDescription.isEmpty ? 'No description provided' : product.shortDescription, style: const TextStyle(fontSize: 14, color: Color(0xFF475569))),
              ),
              const SizedBox(height: 16),
              const Text('Detailed Description', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF64748B))),
              const SizedBox(height: 8),
              Container(
                width: double.infinity,
                padding: EdgeInsets.all(isSmallScreen ? 12 : 16),
                decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(12)),
                child: Text(product.detailedDescription.isEmpty ? 'No description provided' : product.detailedDescription, style: const TextStyle(fontSize: 14, color: Color(0xFF475569))),
              ),
            ],
          ),
        ),
        ...
        if (product.tags.isNotEmpty)
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Tags', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                const SizedBox(height: 16),
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: product.tags.map((tag) => Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: const Color(0xFF7C3AED).withOpacity(0.1), borderRadius: BorderRadius.circular(20)), child: Text(tag, style: const TextStyle(fontSize: 13, color: Color(0xFF7C3AED))))).toList(),
                ),
              ],
            ),
          ),
        ...
        if (product.multipleImages.isNotEmpty)
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Additional Images', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                const SizedBox(height: 16),
                SizedBox(
                  height: 120,
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => Container(
                      ...
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(12),
                        child: Image.network(product.multipleImages[index], fit: BoxFit.cover, width: 120, height: 120, errorBuilder: (context, error, stackTrace) => Container(color: const Color(0xFFF8FAFC), child: const Icon(Icons.broken_image, color: Colors.grey, size: 40))),
                      ),
                    ),
                  ),
                ),
        ...
        if (product.specifications.isNotEmpty)
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Product Specifications', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                const SizedBox(height: 16),
                ListView.separated(
                  shrinkWrap: true,
                    ...
                    return Padding(
                      ...
                      child: isSmallScreen
                          ? Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(entry.key, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Color(0xFF1E293B))),
                                const SizedBox(height: 4),
                                Text(entry.value.toString(), style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
                              ],
                            )
                          : Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                SizedBox(width: 160, child: Text(entry.key, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: Color(0xFF1E293B)))),
                                const SizedBox(width: 16),
                                Expanded(child: Text(entry.value.toString(), style: const TextStyle(fontSize: 13, color: Color(0xFF64748B)))),
                              ],
                            ),
                    );
                  },
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Additional Information', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
              const SizedBox(height: 16),
              _buildAdditionalInfoRow('Product ID', product.id, Icons.qr_code, isSmallScreen),
              const SizedBox(height: 12),
              _buildAdditionalInfoRow('Rating', '${product.rating} ⭐ (${product.totalReviews} reviews)', Icons.star_outline, isSmallScreen),
              const SizedBox(height: 12),
              _buildAdditionalInfoRow('Created Date', _formatDate(product.createdAt), Icons.calendar_today_outlined, isSmallScreen),
              const SizedBox(height: 12),
              _buildAdditionalInfoRow('Status', product.isActive ? 'Active' : 'Inactive', Icons.toggle_on_outlined, isSmallScreen),
            ],
          ),
        ),
      ],
  ...
  Widget _buildAdditionalInfoRow(String label, String value, IconData icon, bool isSmallScreen) {
    if (isSmallScreen) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [Icon(icon, size: 18, color: const Color(0xFF7C3AED)), const SizedBox(width: 12), Text(label, style: const TextStyle(fontSize: 13, color: Color(0xFF64748B)))]),
          const SizedBox(height: 8),
          Padding(padding: const EdgeInsets.only(left: 30), child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF1E293B)))),
        ],
      );
    }
    return Row(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: _mainBannerImageUrl != null
                            ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(_mainBannerImageUrl!, fit: BoxFit.cover, width: double.infinity))
                            : Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.cloud_upload, size: 40, color: Colors.grey.shade400), const SizedBox(height: 8), Text('Tap to upload main banner image', style: TextStyle(color: Colors.grey.shade600))]),
                      ),
                    ),
                    const SizedBox(height: 24),
                    const Text('Additional Images', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1E293B))),
                    const SizedBox(height: 8),
                    OutlinedButton.icon(onPressed: _pickMultipleImages, icon: const Icon(Icons.add_photo_alternate), label: const Text('Add Images'), style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 12))),
                    const SizedBox(height: 12),
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) => Container(
                            ...
                            child: Stack(
                              children: [
                                ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(_multipleImageUrls[index], fit: BoxFit.cover, width: 100, height: 100)),
                                Positioned(
                                  top: 4,
                                  right: 4,
                                  child: GestureDetector(
                                    onTap: () => _removeMultipleImage(index),
                                    child: Container(decoration: BoxDecoration(color: Colors.black.withOpacity(0.5), shape: BoxShape.circle), child: const Icon(Icons.close, size: 16, color: Colors.white)),
                                  ),
                                ),
                              ],
                            ),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(labelText: 'Tags (comma separated)', prefixIcon: Icon(Icons.local_offer_outlined), border: OutlineInputBorder(), helperText: 'e.g., popular, new, sale'),
                          ),
                        ),
                      ],
                    ),
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              TextButton.icon(onPressed: _addSpecification, icon: const Icon(Icons.add, size: 18), label: const Text('Add Specification'), style: TextButton.styleFrom(foregroundColor: const Color(0xFF7C3AED))),
                            ],
                          ),
                          const SizedBox(height: 16),
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Row(
                                  children: [
                                    ...
                                    IconButton(onPressed: () => _removeSpecification(index), icon: const Icon(Icons.delete_outline, color: Colors.red), tooltip: 'Remove'),
                                  ],
                                );
                              },
                            ),
                    ...
                    Row(
                      children: [
                        Expanded(child: OutlinedButton(onPressed: widget.onSuccess, style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Cancel'))),
                        const SizedBox(width: 16),
                        Expanded(
...
class _EditProductFormWidget extends StatefulWidget {
  ...
  final VoidCallback onSuccess;
  const _EditProductFormWidget({required this.product, required this.onSuccess});
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: _mainBannerImage != null
                            ...
                            : ClipRRect(
                                borderRadius: BorderRadius.circular(12),
                                child: Image.network(widget.product.mainBannerImage, fit: BoxFit.cover, width: double.infinity, errorBuilder: (context, error, stackTrace) => Container(color: const Color(0xFFF8FAFC), child: const Icon(Icons.broken_image, color: Colors.grey, size: 40))),
                              ),
                      ),
                    ),
                    if (_mainBannerImage == null) Padding(padding: const EdgeInsets.only(top: 8), child: Text('Tap to change image', style: TextStyle(fontSize: 12, color: const Color(0xFF7C3AED)))),
                    const SizedBox(height: 24),
                    const Text('Additional Images', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF1E293B))),
                    const SizedBox(height: 8),
                    OutlinedButton.icon(onPressed: _pickMultipleImages, icon: const Icon(Icons.add_photo_alternate), label: const Text('Add More Images'), style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 12))),
                    const SizedBox(height: 12),
                          ...
                          itemBuilder: (context, index) => Container(
                            ...
                            child: Stack(
                              children: [
                                ClipRRect(
                                  borderRadius: BorderRadius.circular(8),
                                  child: Image.network(_existingMultipleImages[index], fit: BoxFit.cover, width: 100, height: 100, errorBuilder: (context, error, stackTrace) => Container(color: const Color(0xFFF8FAFC), child: const Icon(Icons.broken_image, color: Colors.grey, size: 30))),
                                ),
                                Positioned(
                                  top: 4,
                                  right: 4,
